...
function folder_query_video(id_folder, path) {
    ...
    do {
        scroll_to_end = window.document.querySelector(".PEfnhb.NtyuW");
        temp = scroll_to_end.scrollHeight;
        scroll_to_end.scrollBy(0, scroll_to_end.scrollHeight);
        do {
            iimPlayCode("WAIT SECONDS=1");
        } while (window.document.querySelector(".n-Jb.n-Ni").offsetParent != null);
    } while (temp != scroll_to_end.scrollHeight);
    ...
    let list_folder = [];
    let list_video = [];
    if (data_bucketnames.indexOf('Thư') != -1) {
        i++;
        let j = 1;
