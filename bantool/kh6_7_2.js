...
function get_result_exam(id, path) {
    ...
    if (tm.querySelector("ul") == null) {
        ...
        if (tm2[1].getElementsByTagName("span")[1].textContent.trim() == "") {
            ...
            window.location.href = tm2[2].querySelector("a").href;
        }
    }
    ...
    try {
        window.document.querySelector("#breadcrumb").remove();
    } catch (e) {}
    try {
        window.document.querySelector(".categorySectionTitle").remove();
    } catch (e) {}
    try {
    ...
    while (true) {
        try {
            window.document.querySelector(".quizContentQuestions>a").remove();
        } catch (e) {
            break;
        }
    }

    cau = window.document.querySelectorAll(".quizContentQuestions>.questionItem");
    ...
    while (true) {
        ...
        iimPlayCode(
            ...
            'TAG POS=1 TYPE=DIV ATTR=CLASS:quizContentQuestions CONTENT=EVENT:SAVE_ELEMENT_SCREENSHOT'
        );
...
function get_chuong(dom_html, path) {
    ...
    while (x < chuong.length) {
        ...
        if (chuong[x].querySelector(".collapse") == null) {
            break;
        }
        let name_chuong = "";
